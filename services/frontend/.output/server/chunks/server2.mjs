import { d as config } from './server.mjs';
...
function encodeQueryValue(text) {
  return encode(text).replace(PLUS_RE$1, "%2B").replace(ENC_SPACE_RE, "+").replace(HASH_RE, "%23").replace(AMPERSAND_RE, "%26").replace(ENC_BACKTICK_RE, "`").replace(ENC_CURLY_OPEN_RE, "{").replace(ENC_CURLY_CLOSE_RE, "}").replace(ENC_CARET_RE, "^");
}
...
(function(exports) {
 ...
 ${JSON.stringify(location2)}${currentLocation ? "\nwhile being at\n" + JSON.stringify(currentLocation) : ""}`;
  ...
  function createRouter(options) {
    ...
    const parseQuery$12 = options.parseQuery || parseQuery2;
    ...
    function resolve(rawLocation, currentLocation) {
      ...
      if (typeof rawLocation === "string") {
        const locationNormalized = parseURL2(parseQuery$12, rawLocation, currentLocation.path);
      ...
      if ("path" in rawLocation) {
        matcherLocation = assign({}, rawLocation, {
          path: parseURL2(parseQuery$12, rawLocation.path, currentLocation.path).path
        });
      } else {
    ...
    function locationAsObject(to) {
      return typeof to === "string" ? parseURL2(parseQuery$12, to, currentRoute.value.path) : assign({}, to);
    }
...
const components = {};
function components_24689fe0(nuxtApp) {
...
const vueuseHead_dd4dd0d2 = defineNuxtPlugin((nuxtApp) => {
...
const plugin_6c7244ea = defineNuxtPlugin((nuxtApp) => {
...
const router_240af46f = defineNuxtPlugin((nuxtApp) => {
  const routes = [];
  const hooks = {
    "navigate:before": [],
    "resolve:before": [],
